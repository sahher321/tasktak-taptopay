...
import { APP_URL, DeleteLeads, GetAllLeads, GetAllProjects } from "../Components/Api";
...
const Lead = () => {
    ...
    const apiPost = async (s) => {
        ...
        axios.get(APP_URL.BaseUrl + GetAllLeads, { "keysearch": "" })
            .then(data => {
                setLoading(false)
    ...
    return (
        <View style={{ flex: 1, marginTop: '5%', alignItems: "center" }}>
            <View style={{ flexDirection: "row", width: "90%" }}>
                ...
                <TouchableOpacity style={{ alignItems: "flex-end", flex: 1 }} onPress={() => handleNewLeadPressed("add", "")}>
                    <Text style={{ fontSize: 20, color: AppTheme.PrimaryColor }}>Add Lead</Text>
                </TouchableOpacity>
        ...
        <TouchableOpacity onPress={projectTap}>
            <View style={{ width: ScreenWidth / 1.0 - 50, borderColor: AppTheme.BorderColor, borderWidth: 1, borderRadius: 8, marginTop: 16, marginEnd: 10 }}>
                ...
                <View style={{ marginLeft: 12, marginTop: 12, flexDirection: 'row' }}>
                    <Text style={{ flex: 1, marginLeft: 5, fontSize: 14, fontStyle: "normal", alignSelf: "center" }}>{"Name: " + item.name}</Text>
                    ...
                    <View style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: 12, backgroundColor: AppTheme.PrimaryColor, marginRight: 15, alignSelf: "center" }}>
                        <Text style={{ fontSize: 12, fontStyle: "normal", color: "white", alignSelf: "center", justifyContent: "center" }}>{item.status == "3" ? "Verified" : item.status == "4" ? "New" : "Contact"}</Text>
                    </View>


                </View>

                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Company: " + item.company || "N/A"}</Text>
                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Phone: " + item.phonenumber || "N/A"}</Text>
                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{item.lm_follow_up == "0" ? "Next Follow up: No" : "Next Follow up: Yes" || "N/A"}</Text>



                <Text style={{ marginTop: 12, marginLeft: 14, fontSize: 12, fontStyle: "normal" }} >{<HTML source={{ html: "Description: " + item?.description }} />}
                </Text>
                ...
                <View style={{ marginLeft: 16, marginTop: 12, flexDirection: 'row', marginBottom: 10 }}>
                    ...
                    <Text style={{ marginLeft: 5, fontSize: 12, fontStyle: "normal", alignSelf: "center" }}>{item.dateadded}</Text>
