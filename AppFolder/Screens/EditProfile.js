...
import { View, StyleSheet, Text, FlatList, Image, TextInput, TouchableOpacity, ScrollView } from "react-native";
import { Navigation } from "react-native-navigation";
import { AppTheme } from "../AppTheme/AppTheme";
import { APP_URL, editProfile, GetAllInvoices } from "../Components/Api";
import { CustomTextfield, CustomTextfieldWithBorder, ScreenWidth } from "../Components/General";
...
const EditProfile = () => {
    ...
    useEffect(() => {
        ...
        setName(UserPermissions.user_data[0]?.firstname + " " + UserPermissions.user_data[0]?.lastname)
        ...
        console.log("UserPermissions.user_data[0].profile_image", UserPermissions.user_data[0].profile_image)
    }, [])
    ...
    return (
        <View style={{ flex: 1, alignItems: "center" }}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                <Text style={{ fontSize: 22, marginLeft: 16, marginTop: 20, alignSelf: "flex-start" }}>Profile</Text>

                <TouchableOpacity onPress={() => {

                    UserPermissions.removeItemValue('user-data')
                    UserPermissions.setUserData({})
                    Navigation.setStackRoot("AppStack", { component: { name: "Tutorial", options: { topBar: { visible: false } } } })

                }}>

                    <Text style={{ fontSize: 22, marginRight: 16, marginTop: 20, alignSelf: "flex-start", color: "red" }}>Logout</Text>
                </TouchableOpacity>
            ...
            >
                <Image style={{ width: 145, height: 145, marginLeft: 10, borderRadius: 72.5 }} source={{ uri: imageData.uri != "" ? imageData.uri : require("../Assets/avartor.png") }} />
            </TouchableOpacity>
            <Text style={{ fontSize: 26, marginLeft: 16, marginTop: 20 }} >{UserPermissions.user_data[0]?.firstname + " " + UserPermissions.user_data[0]?.lastname}</Text>
            <Text style={{ fontSize: 18, marginLeft: 16, marginTop: 10, color: "#242134" }}>{UserPermissions.user_data[0]?.department}</Text>
            <ScrollView style={{ width: ScreenWidth, alignSelf: "center" }}>

                <CustomTextfieldProfile value={name} setValue={setName} placeholder={"User name"} fieldName={"User Name"} marginTop={50} />
                <CustomTextfieldProfile value={email} setValue={setEmail} placeholder={"Email"} fieldName={"Email"} marginTop={20} />
                <CustomTextfieldProfile value={phone} setValue={setPhone} placeholder={"Phone"} fieldName={"Phone"} marginTop={20} />
                ...
                >
                    ...
                    <View style={{ padding: 10, borderRadius: 5, backgroundColor: AppTheme.PrimaryColor, alignSelf: "center", marginTop: 50 }}>
                        <Text style={{ fontSize: 18, fontStyle: "normal", color: "white", alignSelf: "center", justifyContent: "center" }}>Update</Text>
                    </View>
...
const CustomTextfieldProfile = memo(({ placeholder, fieldName, marginTop, value, setValue }) => {
    ...
    return (
        ...
        <View style={{ marginTop: marginTop, alignSelf: "center" }}>
            ...
            <View style={{ alignItems: "center", flexDirection: "row", width: "90%", height: 40, borderRadius: 5, borderColor: "lightgray", borderBottomWidth: 1 }} >
