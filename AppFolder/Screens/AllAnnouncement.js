...
import { View, StyleSheet, Text, FlatList, Image, TextInput, WebView } from "react-native";
...
const AllAnnouncement = () => {
    ...
    return (
        <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 22, marginLeft: 16, marginTop: 20 }}>Announcements</Text>
            ...
            <FlatList style={{ marginTop: 20 }} renderItem={AnnouncementView} data={getAllAssigment}></FlatList>
...
const AnnouncementView = ({ item }) => {
    ...
    return (
        <View style={{ flexDirection: "row", flex: 1, borderBottomColor: AppTheme.BorderColor, borderBottomWidth: 1 }}>
            <Image style={{ marginTop : 22, width: 40, height: 40, marginLeft: 16 }} resizeMode="stretch" source={require("../Assets/ic_circle.png")} />
            <View style={{}}>

                <Text style={{ fontSize: 18, marginLeft: 10, marginTop: 20 }}>{item.name}</Text>
                <Text style={{ fontSize: 15, marginLeft: 10, marginTop: 3, marginRight: 80, marginBottom: 20 }}>{ <HTML source={{html: item?.message}}/>}</Text>
