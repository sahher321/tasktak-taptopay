    ...
    <ScrollView>
      <View style={{ flex: 1, alignItems: "center" }}>
        ...
        >
          ...
          <Text style={{ fontSize: 14, fontWeight: "normal", lineHeight: 30 }}>
            Default Language: {item?.default_language || "-"}
          </Text>
          <Text style={{ fontSize: 14, fontWeight: "normal", lineHeight: 30 }}>
          ...
          <Text style={{ fontSize: 14, fontWeight: "normal", lineHeight: 30 }}>
            Public: {item?.is_public == "0" ? "No" : "Yes"}
          </Text>
        </View>
      </View>
    </ScrollView>
...
const ThirdRoute = ({ props }) => {
  ...
  const apiPostTask = async () => {
    ...
    axios
      .get(APP_URL.BaseUrl + GetTasksByType + props.item.id + "/" + "lead")
      .then((data) => {
        console.log(
          `Task DATA Leads New = ${JSON.stringify(
            data.data,
            null,
            2
    ...
    <View>
      ...
      {selectedItems.length != 0 && (
        <View>
          <ScrollView
            horizontal
            style={{ marginVertical: 20, height: 130, width: ScreenWidth }}
          >
            <View
              style={{
                flexDirection: "row",
              ...
              {selectedItems.map((item, index) => {
                ...
                const source =
                  item?.type == "image/jpeg"
                    ? { uri: item?.uri }
                    : require("../Assets/Dashboard/ic_3.png");

                function removeItem() {
                  let indexOf = selectedItems.findIndex(
                    (val) => val.uri == item.uri
                  );
                  console.log(indexOf);
                  if (indexOf != -1) {
        ...
        <View style={{ marginLeft: 16, marginTop: 12, flexDirection: "row" }}>
          ...
          <View
            style={{
              ...
              borderRadius: 4,
              backgroundColor:
                item.status == 5 ? "#868695" : item == 4 ? "#8C5382" : "red",
              marginRight: 15,
              alignSelf: "center",
            }}
          >
