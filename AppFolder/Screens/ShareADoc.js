...
const ShareADoc = () => {
  ...
  const apiPost = async (s) => {
    ...
    axios
      .get(APP_URL.BaseUrl + GetAllDepartments)
      .then((data) => {
        console.log(
          `Deparments DATA = ${JSON.stringify(data.data.data[0].name, null, 2)}`
        );
        // setApiData([])
  ...
  var date = moment().utcOffset("+05").format("MMM d, yyyy");
  ...
  return (
    <View style={{ backgroundColor: "", flex: 1 }}>
      ...
      <DropPicker
        data={dataAllDeparments.map((item, index) => {
      ...
      <TextInput
        ...
        placeholder={"Share documents,ideas"}
      />
