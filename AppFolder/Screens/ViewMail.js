...
const ViewMail = (props) => {
  ...
  const getMailReply = () => {
    try {
      ...
      fetch(
        APP_URL.BaseUrl + `/api/StaffMailbox/getReplyMail?id=${mailDetail?.id}`,
        requestOptions
      )
  ...
  const getForwardMail = () => {
    try {
      ...
      fetch(
        APP_URL.BaseUrl +
          `/api/StaffMailbox/getforwardMail?id=${mailDetail?.id}`,
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => {
  ...
  const sendReplyHandle = () => {
    try {
      ...
      fetch(
        APP_URL.BaseUrl + "/api/StaffMailbox/sendReplyEmail",
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => {
  ...
  const forwardMailHandle = () => {
    try {
      ...
      fetch(
        APP_URL.BaseUrl + "/api/StaffMailbox/sendForwordEmail",
        requestOptions
      )
        .then((response) => response.json())
        .then((result) => {
    ...
    <View style={styles.maincontainer}>
      <ScrollView>
        <View style={{ padding: 16 }}>
          <Text style={styles.labelText}>{mailDetail?.subject}</Text>
          <View style={{ flexDirection: "row", marginTop: 26 }}>
            ...
            >
              <Text style={{ color: "#000", fontFamily: "Prompt-Medium" }}>
                {"From: " + mailDetail?.from_email}
              </Text>
              <Text style={{ color: "#000", fontFamily: "Prompt-Medium" }}>
                {"To: " + mailDetail?.to_email}
              </Text>
            </View>
          </View>
          <View style={{ marginTop: 16 }}>
            <HTML
              ...
              tagsStyles={{
                p: { color: "#000", fontFamily: "Prompt-Light", fontSize: 14 },
              }}
            />
          </View>
          {mailReply === null ? null : (
          ...
          {forwardMail === null ? null : (
            ...
            >
              <Text style={[styles.labelText, { marginTop: 18 }]}>Forward</Text>
