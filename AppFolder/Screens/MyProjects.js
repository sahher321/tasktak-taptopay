...
const FirstRoute = ({ index }) => {
    ...
    const permissions = UserPermissions?.capabilities["projects"]?.permissions || []
            ...
            <TouchableOpacity onPress={projectTap}>
                <View style={{ width: ScreenWidth / 1.0 - 50, borderColor: AppTheme.BorderColor, borderWidth: 1, borderRadius: 8, marginTop: 16, marginEnd: 10 }}>
                    ...
                    <View style={{ marginLeft: 16, marginTop: 12, flexDirection: 'row', marginBottom: 10 }}>
                        ...
                        <Text style={{ marginLeft: 5, fontSize: 12, fontStyle: "normal", alignSelf: "center" }}>{moment(item.start_date).format("DD-MM-YYYY")}</Text>
    ...
    const apiPost = async (s) => {
        ...
        axios.get(APP_URL.BaseUrl + GetAllProjects, { "keysearch": "" })
            .then(data => {
                ...
                ListHeaderComponent={
                    <>
                        <FlatList
                            ...
                            // data={[
                            //     { title: "Task Tak Social Banners" },
                            //     { title: "Modules QA" },
                            //     { title: "Task Tak Mobile App" },
                            //     { title: "HR Management" },

                            //     // "Task Tak Social Banners", "Modules QA", "", "4"
                            // ]}
                        ...
                        {/* <Text style={{ flex: 1, alignSelf: "flex-start", fontSize: 20, marginTop: 20 }}>My Tasks</Text> */}
                        {/* <TouchableOpacity style={{ alignSelf: "flex-end", fontSize: 17, color: AppTheme.yellowButtonColor, marginRight: 10 }}>
                                <Text style={{ alignSelf: "flex-end", fontSize: 17, color: AppTheme.yellowButtonColor, marginRight: 10 }} onPress={() => {
                                    Navigation.push("AppStack", { component: { name: "AllProjects", options: { topBar: { visible: true } } } })
                                }}>
                                    See More
                                </Text>
                            </TouchableOpacity> */}
                        {/* //   <Text style={{ alignSelf: "flex-end", fontSize: 17, color: AppTheme.yellowButtonColor, marginRight: 10 }}> </Text> */}
                        {/* </View> */}
                    </>
                }
            />
...
const MyProjects = () => {
    ...
    const permissions = UserPermissions?.capabilities["Project"]?.permissions || []
    const isDeletable = permissions.indexOf("delete") != -1
    const isEditable = permissions.indexOf("edit") != -1
    const isCreatable = permissions.indexOf("create") != -1
