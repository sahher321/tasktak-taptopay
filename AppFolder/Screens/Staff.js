...
import { APP_URL, DeleteInvoices, DeleteStaffs, GetAllStaffs } from "../Components/Api";
...
const Staff = () => {
    ...
    const apiPost = async (s) => {
        ...
        axios.get(APP_URL.BaseUrl + GetAllStaffs)
            .then(data => {
...
<View style={{ flexDirection: "row", width: "100%" }}>
            ...
            <Text style={{ fontSize: 22, marginLeft: 16, marginTop: 20 }}>Staff</Text>

            <TouchableOpacity style={{ alignItems: "flex-end", flex: 1 , marginTop: 20 , marginRight : 22}} 
            onPress={() => handleNewStaffPressed()}
            >
                <Text style={{ fontSize: 22, color: AppTheme.PrimaryColor }}>Add Staff</Text>
            </TouchableOpacity>

</View>

            <FlatList style={{ marginTop: 20 }}
                renderItem={({ item, index }) => (<StaffView item={item} apiPost={apiPost} />)}
                data={dataAllStaffs}
            />
    ...
    return (
        <View style={{ flexDirection: "row", flex: 1, borderBottomColor: AppTheme.BorderColor, borderBottomWidth: 1, alignItems: "center" }}>
            ...
            <View style={{}}>
                ...
                <Text style={{ fontSize: 15, marginLeft: 5, marginTop: 3, textTransform: "capitalize" }}> Name: {item.firstname}</Text>
                <Text style={{ fontSize: 15, marginLeft: 10, marginTop: 3 }}>Phone: {item.phonenumber ?? "N/A"}</Text>
                <Text style={{ fontSize: 15, marginLeft: 10, marginTop: 3 }}>Email: {item.email ?? "N/A"}</Text>
