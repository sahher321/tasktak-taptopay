...
import { APP_URL, DeleteLeads, GetAllLeads, GetAllProjects, GetAllProposals } from "../Components/Api";
...
const Proposals = () => {
    ...
    const onSearch = (text) => {
        if (text.length > 0) {
            let sorted = dataAllProjects.filter(function (item) {
                console.log("PRO-00000" + item.id)
                let theItem = "PRO-00000" + item.id
    ...
    const apiPost = async (s) => {
        ...
        console.log(`url is =`, GetAllProposals)

        axios.get(APP_URL.BaseUrl + GetAllProposals, { "keysearch": "" })
            .then(data => {
                setLoading(false)
    ...
    return (
        ...
        <View style={{ flex: 1, marginTop: '5%', alignItems: "center" }}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <Text style={{ marginLeft: 0, fontSize: 20 }}>Proposals</Text>
            ...
            <View style={{ marginLeft: 25 }}>
                ...
                <View style={{ alignItems: "center", flexDirection: "row", height: 40, borderRadius: 5, borderColor: "lightgray", borderWidth: 1, marginTop: 20, width: '90%' }} >
                ...
                <FlatList style={{ marginLeft: -10 }}
                    ...
                    ListHeaderComponent={
                        <>
                            <FlatList
                                renderItem={({ item, index }) => (<ProjectDetail item={item} apiPost={apiPost} />)}
    ...
    return (
        ...
        <TouchableOpacity>
            ...
            <View style={{ width: ScreenWidth / 1.0 - 50, borderColor: AppTheme.BorderColor, borderWidth: 1, borderRadius: 8, marginTop: 16, marginEnd: 10 }}>
                ...
                {/* <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Proposal #: " + "PRO-00000" + item.id}</Text> */}


                <Text style={{ flex: 1, marginLeft: 5, fontSize: 14, fontStyle: "normal", alignSelf: "center" }}>{"Proposal #: " + "PRO-00000" + item.id}</Text>
                    ...
                    <View style={{ paddingHorizontal: 10, paddingVertical: 5, borderRadius: 12, backgroundColor: AppTheme.PrimaryColor, marginRight: 15, alignSelf: "center" }}>
                        <Text style={{ fontSize: 12, fontStyle: "normal", color: "white", alignSelf: "center", justifyContent: "center" }}>{item.status == "2" ? "Declined" : item.status == "4" ? "Sent" : "Draft" }</Text>
                    </View>


                </View>

                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Subject: " + item.subject}</Text>


 


                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"To: "  + item.proposal_to || "N/A"}</Text>
                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Total: " + item.symbol + item.total || "N/A"}</Text>

                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Date: "  + item.date || "N/A"}</Text>

                <Text style={{ marginTop: 12, marginLeft: 16, fontSize: 14, fontStyle: "normal" }}>{"Open Till: "  + item.open_till || "N/A"}</Text>
                ...
                <View style={{ flexDirection: "row", marginLeft: 16, marginTop: 6, marginBottom: 12 }}>
                    <TouchableOpacity
                        activeOpacity={.7}
